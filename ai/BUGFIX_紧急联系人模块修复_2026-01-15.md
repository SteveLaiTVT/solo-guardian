...
## Bug 5: 自动优先级可能超过 5 (P1)

### 问题
当优先级稀疏时（如 [1,3,5]），`maxPriority + 1` 会产生 6，超出 1-5 的有效范围。DTO 的 `@Max(5)` 验证无法拦截，因为输入时 priority 是 undefined。

### 修复
改为查找 1-5 范围内最低的可用优先级槽位：
...
### 示例
- 现有优先级 [1, 3, 5] → 新联系人获得 2（而非 6）
- 现有优先级 [1, 2, 3] → 新联系人获得 4

### 文件
- `emergency-contacts.service.ts` - 重写自动优先级逻辑
